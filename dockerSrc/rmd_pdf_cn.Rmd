...
library(showtext)
showtext_auto()
font_add("msyh", "/usr/share/fonts/chinese/msyh.ttc") # 你的中文字体位置

p1 <- pp + ggtitle("安会计师打防结合") + theme(text = element_text(family = 'msyh'))
tmp_png <- tempfile(pattern = "file", fileext = ".png")
knit_out <- sprintf("![](%s)", tmp_png)
ggsave(tmp_png, p1)
# p1
```

## md插入图片
`r knitr::knit(text = paste(knit_out, collapse = '\n'))`
